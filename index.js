...
class NvimRunServer {
      ...
      tools: [
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              width: { type: 'number', description: 'Terminal width (default: 80)' },
              height: { type: 'number', description: 'Terminal height (default: 24)' }
            }
          }
        },
        {
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              session: { type: 'string', description: 'Session name' },
              keys: { type: 'array', items: { type: 'string' }, description: 'Keys to send' }
            },
        ...
        {
          name: 'nvim_lua_file',
          description: 'Execute Lua code from multiline input (avoids escaping issues)',
          inputSchema: {
            type: 'object',
            properties: {
              session: { type: 'string', description: 'Session name' },
              code: { type: 'string', description: 'Lua code to execute (multiline safe)' }
            },
            required: ['session', 'code']
          }
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              session: { type: 'string', description: 'Session name' },
              color: { type: 'boolean', description: 'Include ANSI color codes' }
            },
            required: ['session']
          }
        },
  ...
  async start({ session, width = 80, height = 24 }) {
    ...
    await this.runCommand(`${NVIMRUN_PATH} start ${sessionName} ${width} ${height}`);
  ...
  async luaFile({ session, code }) {
    ...
    try {
      ...
      await this.runCommand(`${NVIMRUN_PATH} keys ${session} ":luafile ${tmpFile}" Enter`);
  ...
  async screen({ session, color = false }) {
    ...
    const output = await this.runCommand(`${NVIMRUN_PATH} screen ${session} ${colorFlag}`);
  ...
  async wait({ session, pattern, timeout = 5 }) {
    try {
      await this.runCommand(`${NVIMRUN_PATH} wait ${session} "${pattern}" ${timeout}`);
      return {
        content: [
          {
            type: 'text',
  ...
  async insert({ session, text }) {
    ...
    await this.runCommand(`${NVIMRUN_PATH} keys ${session} i "${escaped}" Escape`);
